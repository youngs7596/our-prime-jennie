...
import pika
from pika.exceptions import AMQPConnectionError, StreamLostError, ChannelClosedByBroker, IncompatibleProtocolError
...
class RabbitMQPublisher:
    ...
    def _with_connection(self):
        ...
        # 초기 연결 실패 시 재시도 로직 추가 (서비스 시작 시점 Race Condition 방지)
        max_retries = 5
        for attempt in range(1, max_retries + 1):
            try:
                return pika.BlockingConnection(params)
            except (AMQPConnectionError, StreamLostError, IncompatibleProtocolError, OSError) as e:
                if attempt == max_retries:
                    logger.error(f"❌ RabbitMQ 연결 실패 (최대 재시도 초과): {e}")
                    raise
                
                # 지수 백오프: 2s, 4s, 8s, 16s... (최대 10초로 제한)
                sleep_time = min(2 * (2 ** (attempt - 1)), 10)
                logger.warning(
                    f"⚠️ RabbitMQ 연결 실패 (시도 {attempt}/{max_retries}). {sleep_time}초 후 재시도... 오류: {e}"
                )
            ...
            try:
                ...
                def _callback(ch, method, properties, body):
                    ...
                    try:
                        ...
                        # ACK를 먼저 보내서 long-running job에서 연결 끊김으로 인한 
