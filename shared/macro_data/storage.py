...
def load_snapshot_from_db(
    ...
    try:
        ...
        return GlobalMacroSnapshot(
            ...
            rate_differential=float(row[22]) if row[22] is not None else None,
            global_news_sentiment=float(row[23]) if row[23] is not None else None,
            korea_news_sentiment=float(row[24]) if row[24] is not None else None,
            data_sources=json.loads(row[25]) if row[25] else [],
            missing_indicators=json.loads(row[26]) if row[26] else [],
            stale_indicators=json.loads(row[27]) if row[27] else [],
            validation_errors=json.loads(row[28]) if row[28] else [],
            data_freshness=float(row[29]) if row[29] is not None else 0.0,
        )
