...
def set_sentiment_score(
    ...
    Args:
        ...
        stock_name: 종목명 (옵션)
        news_title: 뉴스 제목 (옵션)
        news_date: 뉴스 발행일 (옵션, YYYY-MM-DD 형식)
        redis_client: 테스트용 Redis 클라이언트 (의존성 주입)
    ...
    try:
        ...
        if old_data_json:
            ...
            existing_title = old_data.get('news_title')
            existing_date = old_data.get('news_date')
    except Exception:
        pass
    ...
    data = {
        ...
        "news_title": news_title or existing_title,
        "news_date": news_date or existing_date,
