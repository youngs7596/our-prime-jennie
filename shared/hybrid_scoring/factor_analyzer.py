...
4. 재무 데이터(PER/PBR/ROE) DB 연동
5. 뉴스+수급 복합 조건 분석
6. NEWS_FACTOR_STATS 테이블 채우기
7. Recency Weighting 가중치 반영
...
# SQLAlchemy Repository 지원 (테스트 용이성)
if TYPE_CHECKING:
...
class FactorAnalyzer:
    ...
    def _get_historical_prices(self, 
        ...
        Repository 모드 지원 (SQLAlchemy ORM)
    ...
    def _sanitize_for_db(self, value):
        """
        DB 저장을 위해 NaN/inf 값을 None으로 변환
        """
        import math
        if value is None:
            return None
    ...
    def _get_financial_data(self, stock_codes: List[str]) -> Dict[str, Dict]:
        """
        종목별 분기별 재무 데이터(PER/PBR/ROE) DB에서 조회
        
        GPT 피드백: "PER/PBR/ROE 데이터 입력 없음" 해결
        FINANCIAL_METRICS_QUARTERLY 테이블에서 분기별 데이터 조회
    ...
    def analyze_news_category_impact(self, 
        ...
        """
        logger.info(f"   뉴스 카테고리별 영향도 분석 시작 ({len(stock_codes)}개 종목)")
        ...
        for code in stock_codes:
            ...
            # PRICE_DATE를 date로 변환 (datetime → date)
    ...
    def analyze_disclosure_impact(self,
        ...
        """
        logger.info(f"   공시 영향도 분석 시작 ({len(stock_codes)}개 종목, lookback={lookback_days}일)")
    ...
    def analyze_compound_conditions(self, stock_codes: List[str]) -> List[ConditionPerformance]:
        ...
        4. 거래량 급증 + 외국인 순매수
        """
        logger.info(f"   복합 조건 분석 시작 ({len(stock_codes)}개 종목)")
    ...
    def analyze_factor_with_financials(self,
                                       ...
                                       forward_days: int = 5) -> FactorAnalysisResult:
        """
        재무 데이터를 포함한 팩터 분석 (시점별 매칭)
        
        GPT 피드백: "PER/PBR/ROE 데이터 입력 없음" 해결
        각 날짜에 해당하는 분기의 PER/PBR/ROE 사용
        """
        factor_def = self.FACTOR_DEFINITIONS.get(factor_key)
        if not factor_def:
            raise ValueError(f"Unknown factor: {factor_key}")
