...
⚠️ **[CRITICAL]**
1. 먼저 충분히 생각(<think>...</think>)하세요.
2. **반드시** 생각(Thinking)이 끝난 후, 위 JSON 형식으로 최종 답안을 작성해야 합니다.
3. JSON은 마크다운 코드블록(```json) 없이 순수 텍스트로만 출력하거나, 코드블록 안에 정확히 넣으세요.
...
JSON 응답: {{"score": 숫자, "reason": "반드시 한국어로 작성. 이중 감점 여부 확인 필."}}
(grade 필드는 제외합니다. 점수만 정확히 계산하세요)

⚠️ **Thinking Process**:
1. 먼저 정량 점수를 확인한다.
2. 토론에서 새로운 정보가 나왔는지 확인한다. (RSI 얘기는 무시)
3. 시장 국면을 고려해 가중치를 둔다.
4. 최종 점수를 계산한다. (한국어로 이유 작성)"""
    return prompt.strip()
    return prompt.strip()
