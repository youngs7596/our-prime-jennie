...
## [중요] 당신의 역할: 엄격한 필터링 (Strict Gatekeeper)

**정량 분석은 이미 완료되었습니다.** 당신은 다음 **정성적 요소만** 평가하여 점수를 보정하세요.
최근 관대한 평가로 인해 부적절한 종목이 과다하게 선정되고 있습니다. **보수적인 잣대**를 적용하세요.
...
**기준: 정량 점수를 기반으로 '최대 +10점 / -30점' 범위 내에서 조정**
(호재는 보수적으로, 악재는 매우 민감하게 반영합니다.)

위 정량 점수가 70점이라면:
- **[Strong Positive]** 실적 서프라이즈, 대규모 수주 확정 등 확실한 팩트: +5~10점
- **[Neutral]** 일반적인 뉴스, 기대감, 중립적 분석: **+0점 (점수 유지)** 또는 -5점 (재료 소멸 우려)
- **[Negative]** 악재성 이슈, 단기 급등 피로감: -10~15점
- **[Critical]** 경영진 리스크, 재무 건전성 훼손: **-20~30점 (즉시 탈락 수준으로)**
...
## 점수 조정 범위
- 최대 ±15점 이내로 조정하세요. (극단적인 ±20점 지양)
- 특별한 이슈가 없으면 0점(점수 유지)이 정답입니다.

## 출력 형식 (반드시 준수)
{{"score": 72, "reason": "정량 70점 기반, Bull의 실적 근거가 구체적이나 시장 국면 고려하여 +2점만 조정."}}

⚠️ **[CRITICAL]**
1. 생각 후 반드시 **한 줄 JSON**으로 최종 답안을 작성하세요.
2. reason 안에 줄바꿈(\\n)을 넣지 마세요. 쉼표로 이어서 작성하세요.
3. JSON은 마크다운 코드블록(```json) 안에 넣거나 순수 텍스트로 출력하세요.
4. grade 필드는 제외합니다. score와 reason만 작성하세요."""
    return prompt.strip()
