...
class TestCrawler(unittest.TestCase):
    ...
    def test_crawl_news_for_stock_success(self):
        ...
        with patch("crawler.feedparser") as mock_feedparser:
            ...
            self.assertEqual(docs[0].metadata["stock_code"], "005930")
            self.assertEqual(docs[0].metadata["stock_name"], "Samsung Electronics")
            self.assertIn("hankyung.com", docs[0].metadata["source_url"])
            self.assertIn("Samsung Electronics Hits Record High", docs[0].page_content)

    def test_crawl_news_skips_old_news(self):
    ...
    def test_filter_new_documents(self):
        """Test deduplication against ChromaDB"""
        with patch("crawler.vectorstore") as mock_vectorstore:
            # Docs to check
            ...
            self.assertEqual(new_docs[0].metadata["source_url"], "http://example.com/2")

    def test_process_unified_analysis(self):
        """Test unified analysis flow (sentiment + competitor risk)"""
        with patch("crawler.jennie_brain") as mock_jennie_brain, \
             patch("crawler.database") as mock_database, \
             patch("crawler.session_scope") as mock_session_scope:
