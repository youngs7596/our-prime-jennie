...
# ============================================================================
# Tests: get_sector (DB 있음) — SECTOR_NAVER 우선
# ============================================================================
...
class TestGetSectorWithDb:
    ...
    def test_get_sector_from_naver(self, mock_session_scope, classifier_with_db):
        """SECTOR_NAVER 우선 조회"""
        mock_session = MagicMock()
        mock_row = MagicMock()
        mock_row.sector_naver = '반도체와반도체장비'
        mock_row.sector_kospi200 = '반도체'
        ...
        assert sector == '반도체와반도체장비'

    @patch('shared.sector_classifier.session_scope')
    def test_get_sector_fallback_kospi200(self, mock_session_scope, classifier_with_db):
        """SECTOR_NAVER 없으면 SECTOR_KOSPI200 사용"""
        mock_session = MagicMock()
        mock_row = MagicMock()
        mock_row.sector_naver = None
        mock_row.sector_kospi200 = '반도체'
        mock_session.execute.return_value.first.return_value = mock_row
        mock_session_scope.return_value.__enter__ = MagicMock(return_value=mock_session)
        mock_session_scope.return_value.__exit__ = MagicMock(return_value=False)
...
# ============================================================================
# Tests: get_sector_group
# ============================================================================

class TestGetSectorGroup:
    """대분류 조회 테스트"""

    def test_get_sector_group_from_naver(self, classifier_no_db):
        """네이버 세분류 → 대분류 변환"""
        # 먼저 캐시에 네이버 업종명 설정
        classifier_no_db.sector_cache['005930'] = '반도체와반도체장비'

        group = classifier_no_db.get_sector_group('005930', '삼성전자')

        assert group == '반도체/IT'

    def test_get_sector_group_unmapped(self, classifier_no_db):
        """매핑 없는 섹터는 원본 유지"""
        classifier_no_db.sector_cache['999999'] = '미지의업종'

        group = classifier_no_db.get_sector_group('999999', '테스트')
